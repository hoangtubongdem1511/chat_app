...
const DebugPanel: React.FC<DebugPanelProps> = ({ conversationId }) => {
  ...
  const checkActiveCalls = async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.get(`/calls/check-active?conversationId=${conversationId}`);
  ...
  const cleanupCalls = async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.post('/calls/cleanup', { conversationId });
      setDebugInfo(response.data);
  ...
  const debugAllCalls = async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.get('/calls/debug');
      setDebugInfo(response.data);
